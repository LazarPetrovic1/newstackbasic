<template>
  <div class="central">
      <p style="margin-bottom: 2rem;">
        There is a total of <span class="mark">{{ users.length }}</span> users <span class="mark">{{ items.length }}</span> items <span class="mark">{{ votes.length }}</span> votes and <span class="mark">{{ comments.length }}</span> comments in the database.
      </p>
      ...
      <p style="margin-bottom: 2rem;">Comments' JSON: </p>
      <p v-if="comments.length > 0" class="json">
        <span class="mark">
...
export default {
  ...
  async mounted() {
    ...
    res.data.forEach(async item => {
      ...
      resp = await this.$axios.get(`http://localhost:4500/comments/${item.id}`)
      this.comments = await this.comments.concat(resp.data)
    });
  },
  head() {
    return {
      title: `Basic App | History of comments in database session`,
      meta: [
        { charset: "utf-8" },
